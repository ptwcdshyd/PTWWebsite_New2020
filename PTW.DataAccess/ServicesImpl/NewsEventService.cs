...
namespace PTW.DataAccess.ServicesImpl
{
    public class NewsEventService : DBDataAccess, INewsEventService
    {
        ...
        public List<News> GetNewsDetailsByTitle(string NewsUrlTitle, string LanguageCode)
        {
            CustomCommand command = null;
            List<News> newsList = new List<News>();
            ...
            try
            {
                using (command = new CustomCommand())
                {
                    ...
                    if (dtResult != null )
                    {
                        if (dtResult.Tables[3].Rows.Count > 0)
                        {
                            for (int i = 0; i < dtResult.Tables[3].Rows.Count; i++)
                            {
                                News news = new News();
